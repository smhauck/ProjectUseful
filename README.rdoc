...
Include the appropriate database gem.  For MySQL or MariaDB add the following line to your Gemfile:
...
Project Useful is copyright 2014 by William B. Hauck, http://www.wbhauck.com .
...
The datatbase currently needs to be setup manually.  Login with admin privileges (root) and create a new database for Project Useful.  Then grant permissions to a new database user that you want to own the database.  Replace USER and PASSWORD with appropriate information.
    create databse project_useful;
    grant all on projectuseful.* to 'USER'@'localhost' identified by 'PASSWORD';
